...
##------------------------------------------task4  prediction   softimpute + PCA + rf
set.seed(123)
...
task4_train_y_monoctyes_fc<- readRDS("./task_data/task4/task4_train_y_monoctyes_fc.rds")
task4_y_id = task4_train_y_monoctyes_fc$subject_id

y_data = task4_train_y_monoctyes_fc %>% dplyr::select(-type,-day_0,-day_1)
x_data = data.frame(subject_id = timelist_id[["0"]][,c("subject_id")], time0)

y_data_train = y_data %>% filter(subject_id %in% c(train_id$subject_id,test_id)) %>% filter(subject_id %in% task4_y_id)
...
result$X2.2..Monocytes.D1.FC.Rank = pred_test_order
