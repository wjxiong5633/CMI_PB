...
library(tidyverse)
set.seed(123)
##------------------------------------------task1  prediction   softimpute + pca + rf
...
task1_train_y_IgG14<- readRDS("./task_data/task1/task1_train_y_IgG14.rds") %>% rename("IgG_PT_14" = "IgG_PT")
task1_y_id = task1_train_y_IgG14$subject_id

y_data = task1_train_y_IgG14 %>% dplyr::select(-type,-timepoint,-specimen_id)
x_data = data.frame(subject_id = timelist_id[["0"]][,c("subject_id")], time0)

y_data_train = y_data %>% filter(subject_id %in% c(train_id$subject_id,valid_id)) %>% filter(subject_id %in% task1_y_id)
...
actual_train = traindata_raw %>% 
  mutate(rank = rank(-IgG_PT_14, ties.method = "min")) %>% 
  arrange(subject_id) %>% 
  dplyr::select(subject_id,IgG_PT_14,rank)
...
## read tsv
result <- read.table("./template_sub.tsv", header = TRUE, sep = "\t")
result$X1.1..IgG.PT.D14.titer.Rank = pred_test_order
