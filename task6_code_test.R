...
##------------------------------------------task6  prediction autoencoder + PCA + rf
set.seed(12345)
...
time0 <- readRDS("./imputed_data/R_autoencoder/time0_mean_count.rds")

task6_train_y_CCL3_fc<- readRDS("./task_data/task6/task6_train_y_CCL3_fc.rds")
task6_y_id = task6_train_y_CCL3_fc$subject_id
y_data = task6_train_y_CCL3_fc %>% dplyr::select(-type,-day_0,-day_3,-foldchange_tpm)
x_data = data.frame(subject_id = timelist_id[["0"]][,c("subject_id")], time0)
y_data_train = y_data %>% filter(subject_id %in% train_id$subject_id) %>% filter(subject_id %in% task6_y_id)
...
y = traindata_raw$foldchange_count
data <- X
target <- y
...
actual_train = traindata_raw %>% 
  mutate(rank = rank(-foldchange_count, ties.method = "min")) %>% 
  arrange(subject_id) %>% 
  dplyr::select(subject_id,foldchange_count,rank)
...
result$X3.2..CCL3.D3.FC.Rank = pred_test_order
