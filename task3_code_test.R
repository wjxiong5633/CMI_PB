...
##------------------------------------------task3  prediction   missMDA + PCA + rf
set.seed(123)
...
task3_train_y_monoctyes<- readRDS("./task_data/task3/task3_train_y_monoctyes.rds")
task3_y_id = task3_train_y_monoctyes$subject_id

y_data = task3_train_y_monoctyes %>% dplyr::select(-type,-timepoint,-specimen_id)
x_data = data.frame(subject_id = timelist_id[["0"]][,c("subject_id")], time0) %>% rename("Monocytes_0" = "Monocytes")



y_data_train = y_data %>% filter(subject_id %in% c(train_id$subject_id,test_id)) %>% filter(subject_id %in% task3_y_id)
...
actual_train = traindata_raw %>% 
  mutate(rank = rank(-Monocytes, ties.method = "min")) %>% 
  arrange(subject_id) %>% 
  dplyr::select(subject_id,Monocytes,rank)
...
result$X2.1..Monocytes.D1.Rank = pred_test_order
